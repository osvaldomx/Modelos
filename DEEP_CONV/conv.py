...
if __name__ == '__main__':
    ...
    conv = Sequential()
    conv.add(InputLayer(input_shape=(10, 206)))

    conv.add(Conv1D(20, 10, activation='tanh'))
    conv.add(MaxPooling1D(padding='same'))
    conv.add(Flatten())

    conv.add(Dense(500, activation='tanh'))
    conv.add(Dense(2, activation='softmax'))

    conv.compile(optimizer='sgd',
    ...
    conv.fit(X_train, y_train_cat, validation_data=(X_val, y_val_cat), epochs=1500)

    print(conv.evaluate(X_test, y_test_cat))

    predict = conv.predict_classes(X_test)
